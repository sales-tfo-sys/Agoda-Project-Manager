...
function planFor(row, cols, idx, index, fields, results) {
  ...
  const hidMatch = hid ? idx.byHid.get(hid)?.$id?.value || null : null;
  ...
  return {
    ...
    answer,
    matchedBy: by,
    hidMatch: hidMatch ? String(hidMatch) : null,
...
export async function buildPlan() {
  ...
  const count = {
    ...
    linkable: rows.filter((r) => r.status === "unlinked" && r.hidMatch).length,
...
export async function autolinkByHid(who, { limit = 200 } = {}) {
  ...
  const targets = (plan.rows || []).filter((r) => r.status === "unlinked" && r.hidMatch).slice(0, limit);
  ...
  const left = (after.rows || []).filter((r) => r.status === "unlinked" && r.hidMatch).length;
