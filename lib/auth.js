...
// ページ権限（各ユーザーの閲覧/編集許可）を task_override(scope=pageperm) から読む。
// 認証チェックのたびに問い合わせないよう短時間キャッシュする。
const pagePermsCache = new Map(); // personId -> { data, at }
const PAGEPERM_TTL = 30000;
export async function getPagePerms(personId) {
  if (!personId || !supabaseConfigured()) return null;
  const hit = pagePermsCache.get(personId);
  if (hit && Date.now() - hit.at < PAGEPERM_TTL) return hit.data;
  try {
    const rows = await sb(
      `task_override?scope=eq.pageperm&key=eq.${encodeURIComponent(personId)}&select=data`
    );
    const data = rows?.[0]?.data || null;
    pagePermsCache.set(personId, { data, at: Date.now() });
    if (pagePermsCache.size > 300) pagePermsCache.clear();
    return data;
  } catch {
    return null;
  }
}
export function invalidatePagePerms(personId) {
  if (personId) pagePermsCache.delete(personId);
  else pagePermsCache.clear();
}

// 直近に確認できたセッション。問い合わせが一時的に失敗したときの控え。
// ※ログイン許可を外した人が使えてしまうのは、障害中のこの時間だけ（ミドルウェアと同じ）。
const sessionOk = new Map(); // sid -> { val, at }
...
export async function getSession(sid) {
  ...
  // 一時的な失敗なら1回だけやり直す。それでも駄目なら、少し前に確認できた内容を使う。
  // （以前は失敗をそのまま「未ログイン」にしていたため、Supabase が一瞬遅れただけで
  //  データが読めなくなっていた）
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const val = await lookupSession(sid);
      if (val) {
        if (sessionOk.size > 300) sessionOk.clear();
        sessionOk.set(sid, { val, at: Date.now() });
      } else {
        sessionOk.delete(sid); // 本当に無効と分かったら控えも捨てる
      }
      return val;
    } catch {
  ...
  const hit = sessionOk.get(sid);
  if (hit && Date.now() - hit.at < SESSION_STALE_MAX) return hit.val;
  return null;
}

// 実際の問い合わせ。通信に失敗したら例外を投げる（「無効」とは区別する）。
async function lookupSession(sid) {
  {
    ...
    const pagePerms = await getPagePerms(p.id);
    return {
      sid: s.id,
      email: s.email,
      name: p.name,
...
// 指定ページの「編集」権限が無ければ拒否（ページ単位の編集制御）。
export async function denyUnlessPageEdit(req, pageKey) {
  const { perms } = await getPerms(req);
  if (!perms) return Response.json({ error: "ログインが必要です" }, { status: 401 });
  if (!perms.pages?.[pageKey]?.edit) {
    return Response.json({ error: "編集する権限がありません" }, { status: 403 });
  }
  return null;
}
