...
export async function writeCellsApi(id, gid, cells) {
  ...
  if (!title) title = sheets[0]?.properties?.title;
  if (!title) return { error: "書き込み先のタブが見つかりません" };
  ...
  const res = await fetchRetry(`https://sheets.googleapis.com/v4/spreadsheets/${id}/values:batchUpdate`, {
    method: "POST",
    headers: auth,
    cache: "no-store",
  ...
  if (!res.ok) {
    ...
    if (res.status === 400 || res.status === 404) {
      ...
      return { error: `シートに書き込めません（Sheets API ${res.status}）` };
    }
    return { error: `シートに書き込めません（Sheets API ${res.status}）` };
  }
  return { ok: true };
