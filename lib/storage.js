...
// 2026-09-18 に Pro（合計100GB）へ移ったが、1ファイルの上限は Supabase 側の
// Storage 設定（Upload file size limit・既定50MB）でも決まるため、ここは50MBのまま。
// 上げるときは、Supabase の Storage 設定を先に上げてから、この値を同じにそろえること
// （画面の案内文もこの定数を使う）。
...
export async function folderStat(path, budget = { left: 40 }, depth = 0) {
  ...
  for (const d of folders) {
    // path が空（＝いちばん上）のときに "/名前" にしてしまうと、
    // 先頭の空の階層まで数えに行ってしまい、何も見つからなくなる
    const sub = await folderStat(path ? `${path}/${d.name}` : d.name, budget, depth + 1);
    out.size += sub.size;
    out.files += sub.files;
