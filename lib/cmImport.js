...
//
// 決めごと（利用者と合意した内容）
//   ・紐づけは画面で1件ずつ選ぶ（自動では推測しない）
//   ・同じ HID の施設が複数あることがある（依頼日や内容が違うだけの別レコード）ので、
//     1つの回答を複数のレコードに紐づけられるようにする
...
/**
 * 保存してある結果の形をそろえる。
 *   { recordIds: ["3657","1680"], applied: { "3657": { wrote, labels, at, by } }, … }
 * 昔の形（recordId と wrote が1件ぶん）も読めるようにしておく。
 */
export function normalizeResult(r) {
  if (!r) return null;
  const ids = Array.isArray(r.recordIds)
    ? r.recordIds.map(String).filter(Boolean)
    : r.recordId
    ? [String(r.recordId)]
    : [];
  let applied = r.applied && typeof r.applied === "object" ? { ...r.applied } : {};
  if (!r.applied && r.wrote && r.recordId) {
    applied = { [String(r.recordId)]: { wrote: r.wrote, labels: r.labels || {}, at: r.at, by: r.by } };
  }
  return { ...r, recordIds: ids, applied };
}
...
export async function readResults() {
  ...
  const out = {};
  for (const r of rows || []) out[r.key] = normalizeResult(r.data || {});
  return out;
}
