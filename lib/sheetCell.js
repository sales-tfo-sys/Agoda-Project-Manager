// Googleスプレッドシートの「特定タブ・特定セル」を1つ読むためのヘルパー。
// タブは URL の gid で特定し、“生グリッド”からセルを取り出す。
// （gviz は先頭の空行/空列を詰めて位置がずれるため使わない）
//
// 取得経路は2つ。フォーム回答シート（lib/formSheet.js）と同じ考え方。
//  ① サービスアカウントが設定済み → Sheets API で読む。シートを
//     サービスアカウントに「閲覧者」で共有しておけば非公開のままでよい。
//  ② 未設定 → export?format=csv。この場合だけ「リンクを知っている全員が閲覧可」が要る。
...
// 指定タブ(gid)のセル(cell 例:"C2")の値を文字列で返す。
// 値が空なら null。読めなかったときは理由つきで throw する（呼び出し側で表示する）。
export async function fetchSheetCell(id, gid, cell) {
  if (!id || !cell) return null;
  const pos = parseA1(cell);
  if (!pos) throw new Error(`セル位置「${cell}」を読み取れません（例：C2）`);
  ...
  // ① サービスアカウント経由（非公開シートでも共有されていれば読める）
  ...
  const priv =
    "このシートは非公開です。サービスアカウントに閲覧共有＋環境変数を設定して再デプロイするか、リンク共有にしてください。";
  if (res.status === 401 || res.status === 403) throw new Error(priv);
  if (!res.ok) throw new Error(`シートを取得できません（HTTP ${res.status}）`);
  const text = await res.text();
  if (text.trimStart().startsWith("<")) throw new Error(priv); // 非公開などでHTMLが返った
  return pick(parseCsvGrid(text));
}
