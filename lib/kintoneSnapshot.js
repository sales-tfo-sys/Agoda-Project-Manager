...
// 保存済みスナップショットの中の1レコードだけ差し替える。
// サイトから項目を直したとき、次の「Kintone取込」を待たずに一覧へ反映するために使う。
export async function patchSnapshotRecord(record) {
  const id = record?.$id?.value;
  if (!id) return false;
  const snap = await readSnapshot();
  const payload = snap?.data;
  if (!payload || !Array.isArray(payload.records)) return false;
  const i = payload.records.findIndex((r) => r?.$id?.value === id);
  if (i < 0) return false;
  payload.records[i] = record;
  // 取り込んだ時刻は変えない（Kintone 全体を取り直したわけではないため）
  await writeSnapshot(payload, snap?.fetchedAt);
  return true;
}

// 何件かまとめて差し替える（1件ずつだと、そのたびに9MBを読み書きしてしまうため）
export async function patchSnapshotRecords(records) {
  const list = (records || []).filter((r) => r?.$id?.value);
  if (!list.length) return 0;
  const snap = await readSnapshot();
  const payload = snap?.data;
  if (!payload || !Array.isArray(payload.records)) return 0;
  const pos = new Map(payload.records.map((r, i) => [String(r?.$id?.value), i]));
  let n = 0;
  for (const rec of list) {
    const i = pos.get(String(rec.$id.value));
    if (i == null) continue;
    payload.records[i] = rec;
    n++;
  }
  if (n) await writeSnapshot(payload, snap?.fetchedAt);
  return n;
}

// スナップショットを1行だけ upsert する。
export async function writeSnapshot(payload, fetchedAt) {
  ...
  try {
    await sb("kintone_snapshot?on_conflict=id", {
      ...
      body: {
        id: 1,
        data: packSnapshot(payload),
        // 取り込んだ時刻。1レコードだけ直したときは元の時刻を引き継ぐ
