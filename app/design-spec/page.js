      ...
      <div className="spec-layout">
        ...
        <div className="spec-body">
          ...
          >
            ...
            <P>
              フォーム回答・作業依頼・Ad Hoc の受注数／完了数セルだけは、登録された URL を都度読む。
              いずれも 60 秒のメモリキャッシュを挟む。読み取りは、サービスアカウントが設定されていれば Sheets API（非公開シートを閲覧共有で読める）、
              未設定なら <C>export?format=csv</C>（リンク共有シートのみ）。
            </P>
            <P>
              Sheets API は「1分あたりの読み取り回数」に上限があり、超えると <C>429</C> が返る。対策として
              ①<C>429</C>・<C>5xx</C> は 0.6 / 1.5 / 3 秒待って最大3回やり直す（<C>Retry-After</C> があればそれに従う）、
              ②gid → タブ名の対応を 10 分覚えておき、1回の読み取りにつき API 呼び出しを2回から1回に減らす。
              それでも上限に当たった場合は「1分ほど待ってから開き直してください」というメッセージを返す（この結果はキャッシュに残さない）。
            </P>
              ...
              rows={[
                [
                  ...
                  <>
                    ...
                    タブと、その右の切替・プルダウン（進捗表＝Regular/Ad Hoc・一覧/グラフ・対象年／作業工数表＝一覧/グラフと、一覧なら対応中/完了・対象月、グラフなら対象週）はページヘッダーの中に置く。
                ...
                [
                  ...
                  <>
                    ...
                    表に列が無い項目（目標対応件数・実作業工数・課題・次回アクション・メモ）は、操作列の「詳細」モーダルで編集する。シート連携済みのタスクは、操作列から作業シートを Excel(.xlsx) でダウンロードできる（Google の export URL を開くだけなので API は使わない）。
                    対象年・絞り込みはページヘッダーの左（タイトルの右）、サービスアカウント・編集・タスク追加のボタンはヘッダーの右に置く（ダッシュボードと同じ形。表の上の操作行は無くした）。
                    サービスアカウントのボタンを押すと、シートの共有先メールアドレスと共有手順を出すモーダルが開く（<C>/api/form-config</C>）。
                    タスク列の右端の印は、工数グルーピング・シート連携・進捗グラフの3つ。Regular / Pending は設定なしで必ずグラフに出るので、切り替えではなく印だけを出す。
                  </>,
                  "同上 ＋ /api/kintone-sync",
                ],
                [
                ...
                [
                  ...
                  <>
                    週次の作業リソース。上の段に「メンバー別リソース割合」「Ad Hoc 詳細」（100% 積み上げ棒）を横並び、下の段に「担当者別 内訳」の数値表を中身ぶんの幅で左寄せに置く。
                ...
                [
                  "工数明細（部品）",
                  <>{"—"}（ダッシュボードの「作業工数表」タブ → 一覧）</>,
                  <>
                    日次グリッド（作業内容 × 担当 × 日付）。対象月を選ぶと月まるごとの日付列を出す（土日・祝日は色分け）。
                    「対応中／完了」タブで切り替え。Complete でも期日の月のあいだは対応中に残し、期日の月を過ぎた表から「完了」へ移す（期日が無いものは Complete になった時点で完了）。タブは表示だけで、工数の集計や工数入力には影響しない。作業内容の列幅は文字幅を実測して決める。
                  </>,
                ...
                [
                  ...
                  <>
                    ...
                    1列目「アカウント」は写真＋ログイン時に取得した Google 名＋メール、2列目「担当者名」は登録時の名前。
                    メールと担当者名の書き換えは、誤クリックでの編集を防ぐためヘッダー右上の編集ボタンを押しているあいだだけ受け付ける。
          ...
          >
            ...
            <Ul>
              ...
              <li>
                <b>③ すべての作業（Regular・Ad Hoc・トータル作業時間）</b>：<b>担当者に関係なくメンバー全員</b>が対象なので、①②やシートで行が無い在籍メンバーの行を足す。
                Ad Hoc も作業工数入力では全員が入力できるため、一覧も全員分の行を出す（担当者だけにすると、担当外の人が入力した値が一覧に出ない）。
                除外したメンバーは除外日より前の月だけ出し、オーナー・管理者と除外日が不明な人は出さない。
                ①で担当者がいない作業に置く「担当なし」の仮の行は、全員分の行が足されたら外す。
              </li>
            </Ul>
            <P>
