            ...
            <Tbl
              ...
              rows={[
                [
                  ...
                  <C>/dashboard</C>,
                  <>
                    閲覧専用。<b>スケジュール</b>（連続する2か月を、1つの枠に並べて表示する（月送りと「今月」は枠に1組だけなので、常に連続した月になる）。予定は Ad Hoc タスクの開始日・期日で、入っている日には印と件数を出す。一覧は日／週／月で範囲を切り替えられ、週・月は日付ごとにまとめて表示する）／<b>進捗表</b>（<b>Regular Task / Ad Hoc Task</b> と <b>一覧 / グラフ</b> の2段で切り替える。一覧は Regular Task・Pending のサマリー表、Ad Hoc Task 一覧、プロジェクト進捗（案件タイプ別のステータス×四半期）。グラフは Regular・Ad Hoc の日次推移。Regular / Ad Hoc の選択は一覧とグラフで共通）／<b>作業工数グラフ</b>（週次の作業リソース詳細＝メンバー別 100% 積み上げ棒・Ad Hoc 詳細・担当者別内訳表。既定は今週、無ければ実績のある最新週）／<b>作業工数表</b>（工数明細）の4タブ。
                    タブと、その右の切替・プルダウン（進捗表＝Regular/Ad Hoc・一覧/グラフ・対象年／作業工数グラフ＝対象週／作業工数表＝対応中/完了・対象月）はページヘッダーの中に置く。
                    作業工数グラフ・作業工数表は案件データ（<C>/api/records</C>）を使わないので、その読み込みを待たずに表示する。
                    選択タブは localStorage に保存（<C>agoda-dash-tab</C>／一覧・グラフは <C>agoda-progress-view</C>。旧「全体 / 案件詳細 / 進捗グラフ」の保存値は進捗表として読み、進捗グラフはグラフ側で開く）。
                  </>,
